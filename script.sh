...
listarPalabrasConVocal() {
  if [ "$VOCAL" == "" ]; then
    echo "Primero debe ingresar una vocal (con la opción 7. Ingresar vocal)"
    return 1
  fi

  echo "Listar palabras con solo esa vocal"
  echo "Fecha: `date`"
  echo "Vocal: $VOCAL"
  
  # Filtra las palabras que cumplen con las condiciones
  result="`cat diccionario.txt | grep "^[^aeiou]*$VOCAL[^aeiou]*$"`"
  ...
  # Cantidad de palabras que cumplen con las condiciones
  cumplen="`echo "$result" | wc -l`"

  # Porcentaje de palabras que cumplen con las condiciones, redondeado a 3 decimales (scale=3)
